...
var fallFrequencyMap = [20, 15, 10, 9, 8, 7, 6, 5, 4, 3, 3, 3];
	...
	update: function() {
		...
		if (!gameOver) {
			...
			if (tick % fallFrequency === 0) {
				var randSpeed = Math.ceil(Math.random() * speedCoefficient);
				var randX = ~~(Math.random() * 10) * 50;
				testRain.push(new Falling(randX, -50, randSpeed, img.water));
			}
